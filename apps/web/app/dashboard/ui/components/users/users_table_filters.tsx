...
export default function UsersTableFilters({
  ...
  roleUuids: string[]
  setRoleUuids: React.Dispatch<React.SetStateAction<string[]>>
  roles: RoleDto[]
  perPage: number
}) {
  ...
  const handleSubmit = React.useCallback(
    (querySearch: string, roleIds: string[], perPage: number) => {
      ...
      router.get('/dashboard/users', data, {
        preserveScroll: true,
        preserveState: true,
        replace: true,
        only: ['users'],
  ...
  const isFiltered = (querySearch?.trim()?.length ?? 0) > 0 || (roleUuids?.length ?? 0) > 0
  ...
  const handleSearch = (value: string) => {
    ...
    debouncedSearch(value, roleUuids, perPage)
  }
